class HealerNaliAdren extends SMPNali;
...
{
	local Pawn P;
	local HealerNaliAdrenFX FX;
	local float HealthAmount;
	local Vehicle V;
	local Monster M;
	local HealerNaliAdrenCorona Corona;
	...
	if (P != None && P.Controller.Adrenaline < P.Controller.AdrenalineMax)
	{
		...
		FX = Spawn(class'HealerNaliAdrenFX',,,P.Location,Rotation);
		Corona = Spawn(class'HealerNaliAdrenCorona',Self);
	}
	
	if (SoundChance > Rand(99))
		Self.PlaySound(Sound'satoreMonsterPackv120.Nali.backup2n');
}
